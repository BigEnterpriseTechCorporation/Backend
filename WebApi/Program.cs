...
void ConfigureServices(IServiceCollection s)
{
    ...
    {
        //if(builder.Configuration["useDB"] == "true") options.UseSqlServer(builder.Configuration["ConnectionString"]);
    ...
    s.AddSwaggerGen(c =>
    {
        ...
        if(isProd) c.AddServer(new OpenApiServer { Url = "/api" });
...
void ConfigureMiddleware(WebApplication app)
{
    ...
    app.UseSwagger(c =>
    {
        c.PreSerializeFilters.Add((swaggerDoc, httpReq) =>
        {
            ...
            {
                //rpi.tail707b9c.ts.net
                !isProd? 
                    new OpenApiServer { Url = $"{httpReq.Scheme}://{httpReq.Host.Value}/" } : // dev
                    new OpenApiServer { Url = "https://rpi.tail707b9c.ts.net/api" } // prod
            };
        });
    });
